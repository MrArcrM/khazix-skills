...
运行 fork 子命令前，先把 `SKILL_DIR` 解析为当前加载的 aihot Skill 目录，不使用旧 Mac 用户名或固定宿主目录；工作区默认用：
...
新 Mac 上 Git 只能恢复脚本与路由配置，`lark-cli` profiles、Slack MCP、Wrangler/Cloudflare 登录态仍需重新授权。
...
| 用户字面命中 | 读这个文档 | 流程 | 目标群 |
|---|---|---|---|
| `share-daily` | `./share-daily.md` | JSON → HTML → share-html → 飞书短消息 × 2 + Slack 短消息 × 1 | 飞书「AI 俱乐部核心成员群」+ 飞书「日报群」+ Slack `#inbox` |
| `test-daily`  | `./test-daily.md`  | JSON → HTML → share-html → 飞书短消息（迭代沙盒，跟 share-daily 同管线） | 测试群（Claude Code App） |
| `share-daily-tip` | `./share-daily-tip.md` | 全量 tip → 去掉精选 → score >= 55 → HTML → share-html → 飞书短消息 + Slack 短消息 | 飞书「云崖书院（日报群）」+ Slack `#inbox` |

**说明**：
- 触发本子命令时**不**走上面的"给用户的输出格式 / 工作流"通用章节——子命令格式更严格、目标群更明确
- **模糊触发不算**：必须字面命中 `share-daily` / `test-daily` / `share-daily-tip` 字符串。"分享日报"、"发到分享群"、"发个日报"等模糊表述不触发，按通用流程处理，避免误推送到生产分享群
- `share-daily-tip` 也必须字面命中才触发；它不走日报 daily 端点，而是走 items 全量 `tip`，排除精选并按 score 过滤。详见 `share-daily-tip.md`
- **2026-05-15**：旧 markdown → PDF 流程已下线，test-daily 在测试群迭代稳定的 HTML 流程已 apply 到 share-daily。两条子命令共用同一脚本管线（fetch_daily_with_roles.py / gen_hero.py / render_daily_html.py），唯一差异是分享 target（share-daily → 飞书 × 2 + Slack #inbox、test-daily → 飞书测试群）
- **2026-05-29**：share-daily target 从 1 处扩到 3 处（新增 飞书云崖书院筑基斋 + Slack #inbox）。每个 target 失败不阻断其他。详见 share-daily.md「分享 targets」段
- **2026-06-04**：yunya target 群切换 —— 筑基斋 → 日报群（同一 bot profile `yunya`）。详见 share-daily.md「决策记录」
